...
//
// Parameter   :  Prof        : Profile_with_Sigma_t object array to store the results
...
//                   intrinsic fluid fields
                     if ( TFluIntIdx[p] != IdxUndef )
                     {
                        ...
                        const double field_value     = ( FluIntTime )
                                                     ? ( FluWeighting     *FluidPtr     [ TFluIntIdx[p] ][k][j][i]
                                                       + FluWeighting_IntT*FluidPtr_IntT[ TFluIntIdx[p] ][k][j][i] )
                                                     :                     FluidPtr     [ TFluIntIdx[p] ][k][j][i]  ;
                        OMP_Data      [p][TID][bin] += field_value*Weight;
                        OMP_Data_Sigma[p][TID][bin] += field_value*field_value*Weight;
...
#                          ifdef GRAVITY
                           case _POTE:
                           {
                              ...
                              const double field_value = ( PotIntTime )
...
#                          if ( MODEL == HYDRO )
                           case _VELR:
                           {
                              ...
                              OMP_Data_Sigma[p][TID][bin] += MomR*MomR*dv;
                              OMP_Weight    [p][TID][bin] += Weight;
                              OMP_NCell     [p][TID][bin] ++;
                           }
                           break;
...
#                             endif
                              ...
                              OMP_Data_Sigma[p][TID][bin] += Pres*Pres*Weight;
                              OMP_Weight    [p][TID][bin] += Weight;
                              OMP_NCell     [p][TID][bin] ++;
                           }
                           break;
...
#                             endif // #ifdef DUAL_ENERGY ... else
                              ...
                              OMP_Data_Sigma[p][TID][bin] += Eint*Eint*Weight;
                              OMP_Weight    [p][TID][bin] += Weight;
                              OMP_NCell     [p][TID][bin] ++;
                           }
                           break;
...
//       skip empty bins since both their data and weight are zero
         if ( Prof[p]->NCell[b] > 0L )
         {
             ...
             Prof[p]->Data_Sigma[b] = Prof[p]->Data_Sigma[b]/Prof[p]->Weight[b] - Prof[p]->Data[b]*Prof[p]->Data[b];
             if (Prof[p]->Data_Sigma[b]<0.)
                Prof[p]->Data_Sigma[b] = 0.;
             else
